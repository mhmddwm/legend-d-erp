...
## ACC-PUR-008 — Fix "amount" NOT NULL error on multi-line journal entries
- Same class of issue as ACC-PUR-007: this environment's `journal_entries` table still had the legacy single-line `debit_account`/`credit_account`/`amount` columns as `NOT NULL`, even though migration `006_journal_multiline.sql` was supposed to have dropped that constraint when the schema moved to `journal_entry_lines`. Any multi-line posting (purchase invoice, and now GRN) failed with `null value in column "amount" ... violates not-null constraint`.
- Migration `024_ensure_journal_legacy_columns_nullable.sql`: explicitly drops `NOT NULL` on all three legacy columns, independent of whether migration `006` was ever fully applied. Safe/idempotent — a no-op if the columns are already nullable.
- **Strong recommendation**: this is the second migration-history gap found in this environment (after the missing `123` inventory account). Rather than patching gaps one at a time as they surface, run the full migration sequence (`002` through `024`, in order) against the live database to bring it in sync with the codebase.
