...
def list_journal_entries(
    ...
    account: Optional[str] = Query(None, description="كود أو اسم الحساب (مدين أو دائن)"),
    ...
    if account:
        # يطابق كود الحساب أو اسمه (عربي/إنجليزي)، سواء كان مديناً أو دائناً
        matching_codes = [
    ...
    if amount_from is not None:
        query = query.filter(JournalEntry.amount >= amount_from)
    if amount_to is not None:
        query = query.filter(JournalEntry.amount <= amount_to)
