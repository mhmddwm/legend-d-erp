...
def delete_account(code: str, db: Session = Depends(get_db)):
    ...
    has_children = db.query(Account).filter(Account.parent_code == code).first()
    if has_children:
        raise HTTPException(400, "لا يمكن حذف حساب له حسابات فرعية")

    has_entries = db.query(JournalEntry).filter(
        or_(JournalEntry.debit_account == code, JournalEntry.credit_account == code)
    ).first()
    if has_entries:
...
def list_entries(db: Session = Depends(get_db)):
    return db.query(JournalEntry).order_by(JournalEntry.entry_date.desc(), JournalEntry.id.desc()).all()
...
def create_entry(payload: JournalEntryIn, db: Session = Depends(get_db)):
    if payload.debit_account == payload.credit_account:
        raise HTTPException(400, "لا يمكن أن يكون حساب المدين نفس حساب الدائن")
    for code in (payload.debit_account, payload.credit_account):
        if not db.query(Account).filter(Account.code == code).first():
            raise HTTPException(400, f"الحساب {code} غير موجود")

    entry = JournalEntry(**payload.model_dump(), source_type="manual")
    db.add(entry)
    db.commit()
    db.refresh(entry)
    return entry
...
def delete_entry(entry_id: int, db: Session = Depends(get_db)):
    ...
    if entry.source_type != "manual":
        raise HTTPException(400, "لا يمكن حذف قيد تلقائي ناتج عن مستند مشتريات؛ احذف المستند المصدر بدلاً من ذلك")
    db.delete(entry)
    db.commit()
    return None
